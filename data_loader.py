...
def create_comprehensive_label_hierarchy(surveys):
    """Create hierarchy preserving _Val labels and relationships"""
    hierarchy = {
        ...
        'levels': OrderedDict(),
        'strict_sequences': {},
        'standardized_labels': set()  # Track standardized labels
    }
    
    for survey_idx, (texts, label_pairs) in enumerate(surveys):
        for label_pair_idx, (parent, label) in enumerate(label_pairs):
            is_standardized = label.endswith('_Val')
            
            # Keep original label for hierarchy construction
            core_label = label
            
            # Derive parent considering _Val relationships
            if '>' in core_label:
                parts = core_label.split('>')
                parent_candidate = '>'.join(parts[:-1])
                
                if is_standardized:
                    base_parent = '>'.join(parts[:-1])
                    if base_parent not in hierarchy['structure']:
                        print(f"⚠ Warning: Missing parent for {label} - expected {base_parent}")
                
                parent = parent_candidate if parent_candidate in hierarchy['structure'] else None
            
            # Track standardized labels
            if is_standardized:
                hierarchy['standardized_labels'].add(label)
            
            # Count label occurrences (using original label)
            hierarchy['counts'][label] = hierarchy['counts'].get(label, 0) + 1
            
            # Track first appearance (using original label)
            ...
            # Process hierarchy using core label
            parts = core_label.split('>')
            current_parent = None
            ...
            for depth in range(len(parts)):
                ...
                # Add standardization flag to leaf nodes
                if depth == len(parts) - 1:
                    hierarchy['structure'][current_label]['is_standardized'] = is_standardized
                ...
                if current_parent:
                    # Get all siblings from parent's children
                    siblings = set(hierarchy['structure'][current_parent]['children'].keys())
                    
                    # Remove self from siblings
                    siblings.discard(current_label)
                    
                    # Add bidirectional relationships
                    hierarchy['structure'][current_label]['siblings'].update(siblings)
...
def add_standardization_labels(df):
    """No longer adds `>Val` labels, but still processes the dataframe."""
    return df  # No modifications, just return the data unchanged.

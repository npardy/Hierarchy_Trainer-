...
class SectionClassifierGUI:
    ...
    def view_label_hierarchy(self):
        """Generate and display label hierarchy with filtering option"""
        if not self.train_input_var.get():
            messagebox.showinfo("Info", "Please select a training input file first")
            return

        try:
            # Load data and generate hierarchy if not exists
            from data_loader import load_data
            _, _, hierarchy_info = load_data(self.train_input_var.get())
            self.current_hierarchy_info = hierarchy_info
            
            # Ask about standardized labels
            show_standardized = messagebox.askyesno(
                "Filter Labels", 
                "Show standardized (_Val) labels in hierarchy?"
            )

            # Generate hierarchy text with filtering
            hierarchy_text = self.format_label_hierarchy(
                self.current_hierarchy_info,
                show_standardized
            )
            self.update_hierarchy_display(hierarchy_text)
            
        except Exception as e:
            messagebox.showerror("Error", f"Failed to generate hierarchy: {str(e)}")

    def format_label_hierarchy(self, hierarchy_info, show_standardized=True):
        """Display hierarchy with _Val labels in correct structure"""
        ...
        for label, details in hierarchy_info['structure'].items():
            # Show standardized labels with marker
            standardized_marker = " [STANDARDIZED]" if details.get('is_standardized', False) else ""
            text += f"Label: {label}{standardized_marker}\n"
            ...
            # Verify and complete sibling relationships
            if details['parent']:
                parent_children = set(hierarchy_info['structure'][details['parent']]['children'].keys())
                expected_siblings = parent_children - {label}
                actual_siblings = set(details['siblings'])
                
                # Add missing siblings
                missing_siblings = expected_siblings - actual_siblings
                if missing_siblings:
                    details['siblings'].update(missing_siblings)
                    # Update other siblings' relationships
                    for sibling in missing_siblings:
                        if sibling in hierarchy_info['structure']:
                            hierarchy_info['structure'][sibling]['siblings'].add(label)
    ...
    def validate_standardization(self):
        try:
            ...
            # Count naturally occurring _Val labels
            standardized_labels = df[df['Label'].str.endswith('_Val', na=False)]
            total_labels = len(df)
            standardized_count = len(standardized_labels)
            
            # Calculate validation metrics
            output = [
                "Standardization Check Results:",
                f"- Standardized Labels Detected: {standardized_count}/{total_labels}",
                f"- Coverage: {standardized_count/total_labels*100:.1f}%"
            ]
            
            # Validate parent relationships for _Val labels
            invalid_standardized = []
            for _, row in standardized_labels.iterrows():
                label = row['Label']
                parent_label = '>'.join(label.split('>')[:-1])  # Direct parent
                base_parent = label[:-4]  # Parent without _Val
                
                # Check if either form exists
                parent_exists = parent_label in df['Label'].values
                base_parent_exists = base_parent in df['Label'].values
                
                if not parent_exists and not base_parent_exists:
                    invalid_standardized.append(f"{label} (missing {parent_label} and {base_parent})")
            
            if invalid_standardized:
                output.append("\n⚠ Critical Validation Errors:")
                output.append(f"{len(invalid_standardized)} _Val labels have missing parents:")
                output.extend(invalid_standardized[:3])  # Show first 3 examples
                if len(invalid_standardized) > 3:
                    output.append(f"... ({len(invalid_standardized)-3} more)")
            
            # Fail validation if any invalid _Val labels
            self.validation_status['standardization'] = not bool(invalid_standardized)
            self.toggle_training_button()
            
            # Display results
            self.validation_output.insert(tk.END, "\n".join(output))
            return self.validation_status['standardization']
